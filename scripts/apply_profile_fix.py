...
def update_documents(source_hashes: dict[str, str]) -> None:
    write(
        ROOT / f"{PREFIX}-CONTENT-VERIFICATION.txt",
        """СОДЕРЖАТЕЛЬНАЯ ПРОВЕРКА ПАКЕТА\n\n"
        ...
        "- служебная фраза о локальном пакете удалена из пользовательского интерфейса;\n"
    ...
    write(
        ROOT / f"{PREFIX}-SOURCE-GATE.txt",
        f"""SOURCE GATE — ЕГЭ, МАТЕМАТИКА, ПРОФИЛЬНЫЙ УРОВЕНЬ\n\n"
        "STATUS: SOURCE_GATE_PASSED — READY_FOR_TILDA_TEST\n\n"
        "Локальные официальные источники ФИПИ 2026:\n"
        f"- source/ege-2026-matematika-profil-demoversiya.pdf — SHA-256 {source_hashes['demo']}\n"
        f"- source/ege-2026-matematika-profil-specifikatsiya.pdf — SHA-256 {source_hashes['spec']}\n"
        f"- source/ege-2026-matematika-kodifikator.pdf — SHA-256 {source_hashes['codifier']}\n\n"
        "Подтверждено: 19 заданий, 235 минут, максимум 32 первичных балла; "
        "12 баллов за задания 1–12 и 20 баллов за задания 13–19.\n"
        "Проектный PDF в цепочку приёмки и релизный ZIP не включён.\n"
    )
    write(
        ROOT / f"{PREFIX}-INSTALLATION.txt",
    ...
    write(
        ROOT / f"{PREFIX}-INDEPENDENT-AUDIT-FINAL.txt",
